...
test('backend console actions open a request-log modal with expandable response bodies and running button state', () => {
  ...
  assert.match(page, /const response = await backendsApi\.syncConsole\(backend\.id\)/)
  ...
  assert.match(list, /:disabled="isConsoleSyncRunning\(backend\.id\)"/)
})
