...
test('backend console actions open a request-log modal with expandable response bodies and running button state', () => {
  ...
  assert.match(page, /<th>\{\{ batchSyncFailureMode \? 'Backend' : 'Time' \}\}<\/th>/)
...
test('backends page exposes a toolbar batch sync button and batch sync helper usage', () => {
  ...
  assert.match(page, /syncBackend: \(backend\) => syncBackendConsole\(backend, false\)/)
  assert.match(page, /await backendsApi\.recordConsoleSyncSummary\(/)
  assert.match(page, /success_count: result\.successCount/)
  assert.match(page, /failure_count: result\.failureCount/)
...
test('frontend API and store call the unified backend console sync endpoint', () => {
  ...
  assert.match(api, /options\.audit === false/)
  assert.match(api, /'\/backends\/console\/sync-summary'/)
