...
Token Gate 是一个面向 OpenAI-compatible 与 Anthropic-compatible 上游的透明 AI 网关。
...
- Token Gate 保持请求/响应尽量透明，不做跨协议适配。
...
Scheduler
        ...
        | filter by status / endpoint / model
...
- `internal/proxy`
  - 透明转发、上游鉴权头替换、SOCKS5 egress、SSE 透传。
- `internal/app`
...
backend 的 `endpoints` 使用字符串列表定义能力，支持：
...
- 代理层的 path/query/body/header 透明转发
