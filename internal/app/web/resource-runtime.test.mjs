...
test("requireResourceRenderRuntimeUtils returns the resource render runtime api when app.js dependencies exist", () => {
  const runtime = requireResourceRenderRuntimeUtils(ResourceRenderRuntimeUtils);

  assert.equal(typeof runtime.buildQuickDetailMarkup, "function");
...
test("requireResourceRenderRuntimeUtils throws a clear error when resource render runtime utils are unavailable", () => {
  assert.throws(
    () => requireResourceRenderRuntimeUtils(null),
    /resource-render-runtime\.js.*load.*before app\.js/i,
  );
});

test("requireResourceRenderRuntimeUtils reports exact missing helper names for partial modules", () => {
  assert.throws(
...
test("requireResourceDataRuntimeUtils returns the resource data runtime api when app.js dependencies exist", () => {
  const runtime = requireResourceDataRuntimeUtils(ResourceDataRuntimeUtils);

  assert.equal(typeof runtime.renderProxyOptions, "function");
  assert.equal(typeof runtime.fetchAllCollectionPages, "function");
  assert.equal(typeof runtime.refreshResourceList, "function");
});

test("requireResourceDataRuntimeUtils throws a clear error when resource data runtime utils are unavailable", () => {
  assert.throws(
    () => requireResourceDataRuntimeUtils(null),
    /resource-data-runtime\.js.*load.*before app\.js/i,
  );
});

test("requireResourceDataRuntimeUtils reports exact missing helper names for partial modules", () => {
  assert.throws(
    () => requireResourceDataRuntimeUtils({
      renderProxyOptions() {},
    }),
    /missing ResourceDataRuntimeUtils methods: fetchAllCollectionPages, refreshResourceList/i,
  );
});
...
test("createAppVmContext injects resource render runtime helpers by default", () => {
  const context = createAppVmContext({
    ResourceViewUtils,
    ResourceStateUtils,
    ResourceCrudUtils,
  ...
  assert.equal(context.ResourceRenderRuntimeUtils, ResourceRenderRuntimeUtils);
  assert.equal(
    context.ResourceRuntimeUtils.requireResourceRenderRuntimeUtils,
    requireResourceRenderRuntimeUtils,
  );
});

test("createAppVmContext injects resource data runtime helpers by default", () => {
  const context = createAppVmContext({
    ResourceViewUtils,
    ResourceStateUtils,
    ResourceCrudUtils,
  ...
  assert.equal(context.ResourceDataRuntimeUtils, ResourceDataRuntimeUtils);
  assert.equal(
...
test("app.js fails clearly during startup when resource render runtime utils are missing", () => {
  const context = createAppVmContext({
    ResourceRuntimeUtils: {
      requireResourceViewUtils,
      requireResourceStateUtils,
    ...
    ResourceRenderRuntimeUtils: null,
  });
...
test("app.js fails clearly during startup when resource data runtime utils are missing", () => {
  const context = createAppVmContext({
    ResourceRuntimeUtils: {
      requireResourceViewUtils,
      requireResourceStateUtils,
